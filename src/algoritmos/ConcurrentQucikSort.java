...
public abstract class ConcurrentQucikSort {
	...
	private static class SortTask extends RecursiveAction{
		...
		private SortTask(int[] arr, int low, int high) {
			super();
			this.arr = arr;
			this.low = low;
			this.high = high;
		...
		protected void compute() {
			if(low < high) {	// Condicón de parada - asegura que el algoritmo continúe dividiendo y ordenando subarreglos mientras haya elementos sin ordenar
	            ...
	            // GESTION DE TAREAS
	            SortTask tQsIzq = new SortTask(arr, this.low, partitionIndex-1);	// Ordeno recursivamente el segmento Izquierdo (elementos menores al pivote)
	            SortTask tQsDer = new SortTask(arr, partitionIndex+1, this.high);	// Ordeno recursivamente el segmento Derecho (elementos mayores al pivote)
	            invokeAll(tQsIzq, tQsDer); // Invocar ambas tareas para ejecución paralela
			}
		...
		// Obtiene la posición de la mediana de 3
